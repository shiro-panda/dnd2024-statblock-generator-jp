...
var mon = {
    ...
    customSenses: "",
    languages: [], understandsBut: "", telepathy: 0, customLanguages: "",
...
function UpdateStatblock(moveSeparationPoint) {
    ...
    // AC
    $("#armor-class").html(StringFunctions.FormatString(StringFunctions.RemoveHtmlTags(mon.customAc ? mon.customAc :
        MathFunctions.GetAc(mon.armorName, mon.natArmorBonus, mon.shieldBonus, mon.dexScore))));
    ...
    // 能力値
    let setAbility = (ability, score, saveProf) => {
        let mod = MathFunctions.PointsToBonus(score);
        let profBonus = mon.customCr ? mon.customProf : MathFunctions.GetProfBonus(mon.cr);
        $("#" + ability + "-score").html(StringFunctions.RemoveHtmlTags(score));
        $("#" + ability + "-mod").html(StringFunctions.RemoveHtmlTags(StringFunctions.BonusFormat(mod)));
        $("#" + ability + "-save").html(StringFunctions.RemoveHtmlTags(StringFunctions.BonusFormat(mod + (saveProf ? profBonus: 0))));
    };
    setAbility("str", mon.strScore, mon.saves[0]);
    setAbility("dex", mon.dexScore, mon.saves[1]);
    setAbility("con", mon.conScore, mon.saves[2]);
    setAbility("int", mon.intScore, mon.saves[3]);
    setAbility("wis", mon.wisScore, mon.saves[4]);
    setAbility("cha", mon.chaScore, mon.saves[5]);
...
function ReplaceTags(desc) {
    ...
    //desc = StringFunctions.wrapBrackets(desc);
...
var FormFunctions = {
    ...
    SetForms: function () {
        ...
        $("#custom-prof-input").val(mon.customCr ? mon.customProf : 2);
    ...
    FillInit: function () {
        ...
        $("#init-preview").val(StringFunctions.RemoveHtmlTags(initText));
    },
    ...
    FillAc: function () {
        ...
        $("#ac-preview").html(StringFunctions.RemoveHtmlTags(ac));
    ...
    FillHp: function () {
        ...
        let hpText = hp + "（" + hdNum + "d" + hitDie +
            ((conMod == 0) ? "" : StringFunctions.BonusFormat(hdNum * conMod)) + "）";
        $("#hp-preview").val(StringFunctions.RemoveHtmlTags(hpText));
    ...
    ResetSkillInput: function () {
        for (let i = 0; i < data.skills.length; i++) {
            let skillName = data.skills[i].name;
            $("input[name=" + skillName + "][value=0]").prop("checked", true);
    ...
    ResetDamageTypesForm: function () {
        for (let i = 0; i < data.damagetypes.length; i++) {
            let damagetype = data.damagetypes[i];
            $("input[name=" + damagetype + "][value='n']").prop("checked", true);
        }
    },
    ...
    SetSkillForm: function (skillName, prof) {
        $("input[name=" + skillName + "][value=" + prof + "]").prop("checked", true);
...
var InputFunctions = {
    ...
    AddGearInput: function (gearType) {
        // Insert alphabetically
        GetVariablesFunctions.AddGear($("#gears-" + gearType + "-input").val(),
            $("#gear-" + gearType + "-note-input").val());
...
var GetVariablesFunctions = {
    ...
    GetAllVariables: function () {
        ...
        for (let i = 0; i < data.abilities.length; i++) {
            ...
            mon.saves[i] = $("#" + ability + "-save-input").prop("checked");
        }
        ...
        mon.customSkills = $("#custom-skills-check").prop("checked") ?
        ...
        let customDamagetype = $("#custom-damagetypes-check").prop("checked")
        ...
        mon.customConditions = $("#custom-conditions-check").prop("checked") ?
        ...
        mon.customSenses = $("#custom-senses-check").prop("checked") ?
        ...
        // 脅威度
        mon.cr = $("#cr-input").val();
        mon.customCr = $("#custom-cr-check").prop("checked") ?
            $("#custom-cr-input").val() : "";
    ...
    SetPreset: function (preset) {
        ...
        for (let i = 0; i < data.skills.length; i++) {
            ...
            if (skillName in preset.skills) {
                ...
                let profBonus = CrFunctions.GetProf();
                mon.skills[i] = (preset.skills[skillName] - abilityMod) / profBonus;
            } else {
                mon.skills[i] = 0;
            };
        };
        ...
        if (preset.languages.includes("を解する")) {
            let speaksUnderstandsArr = preset.languages.split("を解する"),
                ...
                understandsLangs = understands[0].replace("と", "、").split("、"),
    ...
    AddGear: function (gearName, note) {
        if (gearName == "") return;
        if (gearName == "*") {
...
var StringFunctions = {
    ...
    GetPropertiesDisplayArr: function () {
        ...
        let vulnerableDisplayArr = [],
            resistantDisplayArr = [],
            immuneDisplayArr = [];
        if (mon.customVulnerable || mon.customResistant || mon.customImmune) { //手動入力
        ...
        /* 選択式にするかもしれないからとりあえず残す
        let gearsDisplayArr;
        for (let i = 0; i < mon.gears.length; i++)
            gearsDisplayArr.push(mon.gears[i].name + mon.gears[i].note);
        gearsDisplayString = gearsDisplayArr.join("、");
        */
        if (mon.customGears) {
            gearsDisplayString = mon.customGears;
        } else {
            let gears = [];
            for (let i = 0; i < mon.gears.length; i++) {
                gears.push(mon.gears[i].name + mon.gears[i].note);
            }
            gearsDisplayString = gears.join("、");
        };

        // 言語
        if (mon.customLanguages) {
            languagesDisplayString = mon.customLanguages;
        } else {
            ...
            languagesDisplayString = speaksLanguages.join("、");
            ...
            languagesDisplayString = StringFunctions.ConcatUnlessEmpty(languagesDisplayString, understandsDisplayString, "；");
        };
        ...
        languagesDisplayString = StringFunctions.ConcatUnlessEmpty(languagesDisplayString, telepathyDisplayString, "；");
        if (!languagesDisplayString) languagesDisplayString = "なし";
        ...
        pushArr("言語：", languagesDisplayString);
    ...
    wrapBrackets(text) {
        return text
            .replace(/（/g, '<span class="punc-open">（</span>')
            .replace(/［/g, '<span class="punc-open">［</span>')
            .replace(/【/g, '<span class="punc-open">【</span>')
            .replace(/）/g, '<span class="punc-close">）</span>')
            .replace(/］/g, '<span class="punc-close">］</span>')
            .replace(/】/g, '<span class="punc-close">】</span>');
    }
}
...
function InitializeEvents() {
    ...
    // 装備
    $(document).on("change", "[data-gears]", function () {
        $("#other-gear-input").toggle($(this).val() == "*");
    });
